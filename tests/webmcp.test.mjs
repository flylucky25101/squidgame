...
test('WebMCP uses real organization actions and validates all inputs', () => {
  ...
  const engine = {
    game: createGame({ cash: 3500 }),
    buy(id) {
      return buyAsset(this.game, id);
    },
  };
  const unregister = registerGameTools(context, engine);
  assert.deepEqual(
    registered.map((t) => t.tool.name),
    ['get_last_city_status', 'purchase_last_city_asset'],
  );
  ...
  unregister();
  assert(registered.every((t) => t.options.signal.aborted));
});
test('unsupported WebMCP contexts do not block the game', () => {
